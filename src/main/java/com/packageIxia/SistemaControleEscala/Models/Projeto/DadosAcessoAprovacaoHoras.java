		...
		for (HoraTrabalhada horaTrabalhada : horasTrabalhadas) {
			...
			for (DadoGenerico dadoGenerico : this.dadosAprovacao) {
				if (dadoGenerico.getId() == horaTrabalhada.getProjetoEscala().getId()) {
					dadoGenerico.setDoubleValue(Utilities.Round(dadoGenerico.getDoubleValue() + (horaTrabalhada.isExcluido() ? 0 : (horaTrabalhada.getTipoAcao() == 1 ? horaTrabalhada.getHoras() : -horaTrabalhada.getHoras())), 3));
					if (horaTrabalhada.getTipoAcao() == 1) {
						this.totalHoras += hora;
						this.totalValor += (hora * 60 * config.getValorMinuto());
