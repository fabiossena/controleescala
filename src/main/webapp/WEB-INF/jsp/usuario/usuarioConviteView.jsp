...
<head>
    ...
    <script src="<c:url value='/js/usuario/usuario-convite.js' />"></script>
</head>
            ...
            <div class="container border-top panel-custom">
                <form:form modelAttribute="usuarioConvite" class="form-horizontal" method='POST'>
                    ...
                    <div class="container row">
                        ...
                        <div class="form-group col-12 col-sm-12 col-md-6 col-lg-6 col-xl-3">
                            ...
                            <div>
                                <form:select 
                                	path="funcao.id" 
                                	id="funcao-id"  
                                	class='form-control ${result.hasFieldErrors("funcao.id") ? "is-invalid" : ""}'  
                                	items="${funcoes}" 
                                	itemLabel="nome" 
                                	itemValue="id"></form:select>
                                <div class="invalid-feedback" id="feedback-funcaoId"></div>
                            </div>
                            <span class="text-success" style="font-size: 10pt" id="perfil-acesso"></span>
                        ...
                        <div class="form-group col-12 col-sm-12 col-md-6 col-lg-6 col-xl-3">
                            <label for="centroCusto.id" class="control-label">Centro de custo</label>
                            <div>
                                <form:select path="centroCusto.id" id="centro-custo-id" class='form-control ${result.hasFieldErrors("centroCusto.id") ? "is-invalid" : ""}'  items="${centroCustos}" itemLabel="descricaoCompleta" itemValue="id"></form:select>
                                <div class="invalid-feedback" id="feedback-centroCustoId"></div>
                            </div>
                        </div>
                        ...
                        <tbody>
                            <c:forEach items="${usuariosConvites}" var="item">
                                <tr id="usuarioAcesso${item.id}">
                                    ...
                                    <td id="centroCusto${item.id}">${item.centroCusto.descricaoCompleta}</td>
                                    <td>
                                        <c:if test="${item.aceito}">Aceito</c:if>
                                        <c:if test="${!item.aceito}">Pendente</c:if>
                                    </td>
                                </tr>
                            </c:forEach>
                        </tbody>
